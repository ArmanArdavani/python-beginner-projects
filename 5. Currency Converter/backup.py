...
def get_inputs():
    while True:
        ...
        while True:
            source_currency = input(f'Source currency {CURRENCIES_PROMPT}: ')
            if source_currency not in (CURRENCIES):
                print("Invalid answer!")
            else:
                break

        # Ask for number of Currencies
        while True:
            try:
        ...
        while True:
            ...
            for currency in range(number_of_curr):
                while True:
                    target_currency = input(f'Target currency {CURRENCIES_PROMPT}: ')
...
def convert(amount, source_currency, target_currencies):
    for target in target_currencies:
        if source_currency == target:
            print(f"{amount} {source_currency} is equal to {amount} {target}")
        else:
            rate = ALL_RATES[source_currency][target]
            print(f"{amount} {source_currency} is equal to {round(amount * rate, 2)} {target}")           



def main():
    amount, source_currency, target_currencies = get_inputs ()
    convert(amount, source_currency, target_currencies)
